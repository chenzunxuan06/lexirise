...
/** F4 每日练习生成 */
export function practiceMessages(pickEntries, distractorPool, unitLabel, gradeLabel) {
  ...
  return [
    { role: "system", content: SYS_CORE },
    {
      ...
      content:
        ...
        '{"questions": [{"type": "fill", "q": "题目文本", "q_zh": "题目中文翻译", "options": ["四选一选项（fill 必填，其他题型为 null）"], "answer": "标准答案", "explain": "一句话解析"}]}\n' +
        "硬性要求：\n" +
        "1. 每道题的英文 q 必须符合【学生年级】水平——只允许出现 目标词/选项词 + 最基础的常用词（I/you/we/the/a/like/go/school/friend/home 等），" +
        "**禁止出现 skiing/amount/professional/necessary 这类超纲词**，句子 ≤ 8 个词，宁可用短的、确定的句子，不要炫技；\n" +
        "2. 每题必须提供 q_zh（中文翻译），帮助理解；\n" +
        "3. 题型 type 只允许三个值：fill（选词填空）、recall（看释义拼单词）、transform（词形变化）。" +
        ...
        "answer 全部小写；fill 的 options 四选一且包含 answer。",
    },
  ];
}
